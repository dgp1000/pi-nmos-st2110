...
<div class="wrap"><table><thead><tr>
 <th title="The flow whose sender pacing is being measured.">flow</th><th class="n" title="ST 2110-21 network-compatibility model: the peak fill of a virtual receive buffer drained at the flow's mean rate, in packets. ~1-5 = narrow (hardware-paced); higher = the sender clumps packets (bursty). Measured from kernel RX timestamps.">Cmax (pkts)</th><th class="n" title="Longest run of back-to-back packets - ones arriving closer together than 20% of the ideal inter-packet interval.">max burst</th><th class="n" title="Mean gap between packet arrivals, microseconds (kernel RX timestamps).">gap mean &micro;s</th><th class="n" title="Largest single gap between arrivals, microseconds - how long the flow goes silent between bursts (e.g. one video frame interval).">gap max &micro;s</th><th title="Receiver-observed pacing class from Cmax: narrow (<=5), wide (<=20), bursty (>20).">pacing</th><th title="The traffic-shaping type the sender declares in its SDP (ST 2110-21 TP). Our software senders honestly declare TP=2110TPW (Wide); true 2110TPN needs a hardware-paced NIC.">declared (SDP)</th>
