...
| IS-11 stream-compat | `pc/is11-nmos.py` | `atoll-is11` | `:8107` | AMWA **IS-11** Stream Compatibility Management API on a self-contained node: it self-hosts its own IS-04 Node API, a **full IS-05 Connection API** (staging, immediate + scheduled relative/absolute activation, `"auto"` transport-param resolution, a live transport file, and bulk activation), and the IS-11 surface (senders + status/constraints/inputs, receivers + status/outputs, inputs + EDID base/effective, outputs + EDID). A grain_rate constraint retunes the flow; a Base-EDID PUT is validated and adjusts the effective EDID. Passes the whole runnable **IS-11-01** suite (92 pass, 0 fail) and **IS-05-01** (62 pass, 0 fail); see §8. |
