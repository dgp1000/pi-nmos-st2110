...
tile_full() {
  ...
  case "$src" in
    ...
    sps) echo "funnel name=fn$idx ! rtpjitterbuffer latency=200 ! rtpmp2tdepay ! tsdemux name=$name udpsrc address=$SPS_A_GRP port=$SPS_A_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! queue ! fn$idx. udpsrc address=$SPS_B_GRP port=$SPS_B_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! queue ! fn$idx. $name. ! h264parse ! queue ! nvh264dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='ST 2022-7 SPS' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-buffers=2 ! mix.sink_$idx $name. ! audio/mpeg ! queue ! fakesink sync=false"; return ;;
