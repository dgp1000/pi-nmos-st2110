...
- `pc/hevc-stream-{env,send,view}.sh` — **4K HEVC GPU pipeline** (the GPU counterpart to JPEG-XS): `videotestsrc 4K59.94 → nvh265enc (NVENC ~50 Mbit CBR) → RTP → multicast 239.10.10.65:5010 → nvh265dec (NVDEC) → cudadownload → GL-downscale to 1080p → glimagesink (D3D12)`. Encode+decode+render all on the RTX 2080 Ti (`nvidia-smi` shows encoder+decoder util, e.g. enc ~34% / dec ~3%). The env auto-detects the island NIC by IP. Window auto-centers. Run: `bash pc/hevc-stream-send.sh` + `bash pc/hevc-stream-view.sh`.
...
- **WSL renames the island NIC across reboots** — it was `eth1` yesterday, `eth0` today (the one holding `10.10.10.2`). NEVER hardcode the iface name; detect it by IP (`ip -o -4 addr show | awk '$4 ~ "^10.10.10.2/"'`). `hevc-stream-env.sh` does this. ⚠️ The older `jxs-stream-*`, `monitor-web.py`, `activation-watcher.py`, `video-web.py` still hardcode the iface and will break on a boot where it's not their assumed name — apply the same IP-detection when next touched.
- **NVIDIA NVDEC/NVENC (CUDA) can't share GPU buffers with the Mesa-D3D12 GL** used by glimagesink — insert `cudadownload` (CUDA→system) before `glupload`/`videoconvert`; the GL scale/render then runs on the D3D12 side. (Zero-copy `nvh265dec ! glupload` fails `not-negotiated`.)
