...
| IS-11 stream-compat | `pc/is11-nmos.py` | `atoll-is11` | `:8107` | AMWA **IS-11** Stream Compatibility Management API on a self-contained node: it self-hosts its own IS-04 Node API, a minimal IS-05 Connection API, and the IS-11 surface (senders + status/constraints/inputs, receivers + status/outputs, inputs + EDID base/effective, outputs + EDID). A grain_rate constraint retunes the flow; a Base-EDID PUT is validated and adjusts the effective EDID. Passes the whole runnable **IS-11-01** suite (92 pass, 0 fail; see §8). |
