...
chk("root members property returns 2 descriptors", isinstance(mem, list) and len(mem) == 2)
...
chk("members are DeviceManager + ClassManager", roles == ["ClassManager", "DeviceManager"])
...
chk("classManager.controlClasses returns 6 classes", isinstance(cc, list) and len(cc) == 6)
...
chk("GetSequenceLength(members) == 2", result(cmd(s, 1, 1, 7, {"id": {"level": 2, "index": 2}})).get("value") == 2)
