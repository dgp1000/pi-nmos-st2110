...
WINW="${ATOLL_TV_W:-3840}"; WINH="${ATOLL_TV_H:-2160}"
PWSH="$(command -v powershell.exe 2>/dev/null || echo /mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe)"
...
tile_full() {
  ...
  case "$src" in
    raw)   echo "$(raw_video "$w" "$h") ! textoverlay text='Pi raw 2110-20' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx"; return ;;
    jpegxs) echo "videotestsrc pattern=ball motion=sweep is-live=true ! video/x-raw,width=$w,height=$h,framerate=30/1 ! videoconvert ! video/x-raw,format=Y42B ! svtjpegxsenc ! svtjpegxsdec ! videoconvert ! textoverlay text='JPEG XS 2110-22' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx"; return ;;
    j2k) echo "udpsrc address=$J2K_GRP port=$J2K_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,encoding-name=JPEG2000,clock-rate=90000,sampling=YCbCr-4:2:0' ! rtpj2kdepay ! avdec_jpeg2000 ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='JPEG 2000' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx"; return ;;
    ...
    h264) echo "udpsrc address=$H264_GRP port=$H264_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=H264,payload=96' ! rtpjitterbuffer latency=100 ! rtph264depay ! h264parse ! nvh264dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='H.264 RTP' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx"; return ;;
    ...
    vp9) echo "udpsrc address=$VP9_GRP port=$VP9_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=VP9,payload=96' ! rtpjitterbuffer latency=100 ! rtpvp9depay ! vp9parse ! nvvp9dec ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='VP9 RTP' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx"; return ;;
    ...
    tsrtp) echo "udpsrc address=$TSRTP_GRP port=$TSRTP_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! rtpjitterbuffer latency=200 ! rtpmp2tdepay ! tsdemux name=$name $name. ! h264parse ! queue ! nvh264dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='TS over RTP' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx $name. ! audio/mpeg ! queue ! fakesink sync=false"; return ;;
    ...
    fec) echo "udpsrc address=$FEC_GRP port=$FEC_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! rtpst2022-1-fecdec name=fd$idx udpsrc address=$FEC_GRP port=$((FEC_PORT+2)) multicast-iface=$IFACE auto-multicast=true caps='application/x-rtp,payload=96,clock-rate=90000' ! queue ! fd$idx.fec_0 udpsrc address=$FEC_GRP port=$((FEC_PORT+4)) multicast-iface=$IFACE auto-multicast=true caps='application/x-rtp,payload=96,clock-rate=90000' ! queue ! fd$idx.fec_1 fd$idx. ! rtpjitterbuffer latency=500 max-misorder-time=5000 max-dropout-time=5000 ! rtpmp2tdepay ! tsdemux name=$name $name. ! h264parse ! queue ! nvh264dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='ST 2022-1 FEC' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx $name. ! audio/mpeg ! queue ! fakesink sync=false"; return ;;
    # ST 2022-7: both paths funnelled into one rtpjitterbuffer, which drops the duplicate copy of
    # each sequence number -- that IS the seamless merge. The interactive path-kill demo is in
    # single view (meter-view.py), which can gate each path live.
    sps) echo "funnel name=fn$idx ! rtpjitterbuffer latency=200 ! rtpmp2tdepay ! tsdemux name=$name udpsrc address=$SPS_A_GRP port=$SPS_A_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! queue ! fn$idx. udpsrc address=$SPS_B_GRP port=$SPS_B_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! queue ! fn$idx. $name. ! h264parse ! queue ! nvh264dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='ST 2022-7 SPS' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx $name. ! audio/mpeg ! queue ! fakesink sync=false"; return ;;
    ...
    mjpeg) echo "udpsrc address=$MJPEG_GRP port=$MJPEG_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=16777216 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=JPEG,payload=96' ! rtpjitterbuffer latency=100 ! rtpjpegdepay ! nvjpegdec ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='MJPEG RTP' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx"; return ;;
  ...
  echo "$(hevc_tile "$grp" "$port" "$w" "$h" "$name") ! ${ss}textoverlay text='$label' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-time=700000000 max-size-buffers=0 max-size-bytes=0 ! mix.sink_$idx$abranch"
}
...
while true; do
  ...
  if [ "$key" != "$cur_key" ]; then
    ...
    if [ -n "$cmd" ]; then
      ...
      pid=$!
      cur_key="$key"
      [ "${ATOLL_PLATFORM:-}" = wsl ] && [ "$SCREEN" != "0" ] && "$PWSH" -NoProfile -ExecutionPolicy Bypass -File "$MOVER" -Screen "$SCREEN" -TimeoutSec 12 >/dev/null 2>&1 &
      [ "${ATOLL_PLATFORM:-}" = wsl ] && [ "$SCREEN" != "0" ] && "$PWSH" -NoProfile -ExecutionPolicy Bypass -File "$SNAP" -Screen "$SCREEN" -TimeoutSec 14 >/dev/null 2>&1 &
    fi
  fi
