...
| IS-12 control | `pc/is12-nmos.py` | `atoll-is12` | `:8108` HTTP, `:8109` ws | AMWA **IS-12** NMOS Control Protocol over a hand-rolled WebSocket (`urn:x-nmos:control:ncp`), carrying the **MS-05-02** object model: a root NcBlock owning an NcDeviceManager + NcClassManager. Generic Get/Set/GetSequenceItem/Length, GetMemberDescriptors, and a class manager serving the canonical MS-05 descriptors (6 classes / 58 datatypes, inheritance merged). Self-hosts its IS-04 Node API. 32/32 on `pc/is12-conformance.py` (see §8). |
