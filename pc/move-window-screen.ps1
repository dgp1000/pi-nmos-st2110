...
# Wayland clients can't position themselves, so we move the window from the Windows side
# after it maps. Title-independent: snapshot existing windows, then grab the NEW one that
# appears (the just-launched viewer) and size+move it to the target screen's bounds.
#
# WSLg often repositions the window to its default spot after the first frame, so we keep
# snapping it back for TimeoutSec. waylandsink (unlike glimagesink) tolerates the resize.
#
...
if (-not $placed) { Write-Output "no new window appeared to place"; exit 1 }
exit 0
