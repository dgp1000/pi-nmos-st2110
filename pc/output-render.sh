...
tile_full() {
  ...
  case "$src" in
    ...
    fec) echo "udpsrc address=$FEC_GRP port=$FEC_PORT multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 caps='application/x-rtp,media=video,clock-rate=90000,encoding-name=MP2T,payload=33' ! rtpst2022-1-fecdec name=fd$idx size-time=4000000000 udpsrc address=$FEC_GRP port=$((FEC_PORT+2)) multicast-iface=$IFACE auto-multicast=true caps='application/x-rtp,payload=96,clock-rate=90000' ! queue ! fd$idx.fec_0 udpsrc address=$FEC_GRP port=$((FEC_PORT+4)) multicast-iface=$IFACE auto-multicast=true caps='application/x-rtp,payload=96,clock-rate=90000' ! queue ! fd$idx.fec_1 fd$idx. ! rtpjitterbuffer latency=200 ! rtpmp2tdepay ! tsdemux name=$name $name. ! h264parse ! queue ! nvh264dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$w,height=$h ! textoverlay text='ST 2022-1 FEC' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! queue leaky=downstream max-size-buffers=2 ! mix.sink_$idx $name. ! audio/mpeg ! queue ! fakesink sync=false"; return ;;
