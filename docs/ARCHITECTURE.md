...
| IS-07 emitter | `pc/is07-tally.py` | `atoll-is07` | `:8102` REST, `:8103` ws | Full IS-07 Events API: one boolean event source per Atoll source key plus **number** (on-air source count), **string** (program label) and **string-enum** (program key) sources derived from the live program bus, so it exercises every IS-07 event type. Serves the REST Events API (CORS + conformant errors) and pushes state over the WebSocket transport on transition. Registers node/device/13 sources/flows/senders in IS-04. **AMWA IS-07-01: 19/19 applicable pass (0 fail).** |
...
| Audio channel map (IS-08) | `pc/audiomap-nmos.py` | `atoll-audiomap` | `:8094` | Serves the AMWA IS-08 v1.0 Channel Mapping API for the music audio (node/device with a `cm-ctrl` control). A controller maps the output's channels to the input's (stereo / swap / mono / mute); on activation it writes `~/atoll-run/audiomap` and restarts the audio mapper. Immediate + scheduled activation, with an output **lock** (`423`) while an activation is pending. Passes AMWA **IS-08-01** 31/0. |
...
| IS-11 stream-compat | `pc/is11-nmos.py` | `atoll-is11` | `:8107` | AMWA **IS-11** Stream Compatibility Management API on a self-contained node: it self-hosts its own IS-04 Node API, a **full IS-05 Connection API** (staging, immediate + scheduled relative/absolute activation, `"auto"` transport-param resolution, a live transport file, and bulk activation), and the IS-11 surface (senders + status/constraints/inputs, receivers + status/outputs, inputs + EDID base/effective, outputs + EDID). A grain_rate constraint retunes the flow; a Base-EDID PUT is validated and adjusts the effective EDID. It also advertises itself over **mDNS/DNS-SD** (`_nmos-node._tcp`, `pc/mdns_responder.py` — a dependency-free responder) for peer-to-peer discovery. Passes the whole runnable **IS-11-01** suite (92/0), **IS-05-01** (62/0), and **IS-04-03** (17/0, including the mDNS discovery test); see §8. |
| IS-12 control | `pc/is12-nmos.py` | `atoll-is12` | `:8108` HTTP, `:8109` ws | AMWA **IS-12** NMOS Control Protocol over a hand-rolled WebSocket (`urn:x-nmos:control:ncp`), carrying the **MS-05-02** object model: a root NcBlock owning an NcDeviceManager, an NcClassManager, and an **NcAtollRigControl** worker whose property Sets **drive the live rig** (cut the program bus, move A/V sync). Generic Get/Set/GetSequenceItem/Length, GetMemberDescriptors, and a class manager serving the canonical MS-05 descriptors plus the vendor class (7 classes / 58 datatypes, inheritance merged). Self-hosts its IS-04 Node API. 40/40 on `pc/is12-conformance.py` (see §8). |
| H.264/H.265 NMOS | `pc/codec-nmos.py` | `atoll-codec-nmos` | `:8110` | AMWA **BCP-006-02** registrar for the rig's H.264 (`atoll-h264`) and H.265 (`atoll-h265`) RTP streams: registers each as a coded-video Flow (`media_type` video/H264 or video/H265) and serves a conformant SDP. It captures the **actual** parameter sets off the live stream -- SPS/PPS for H.264 (`profile-level-id` + `sprop-parameter-sets`), VPS/SPS/PPS for H.265 (`profile-id`/`tier-flag`/`level-id` + `sprop-vps/sps/pps`, de-emulated) -- plus `b=AS` and `ts-refclk`, so the manifest matches the wire. |
...
| `h265` | `atoll-h265` | `pc/h265-send.sh` | NVENC H.265 4 Mb/s | RTP (RFC 7798), pt 96 | `239.10.10.76:5019` |
...
`playback-send.py` -- to the Test Reels group, so you cut to a recording like any other input. The clip list lives behind its own collapsible **Recordings** toggle (like the TV-channel menu), and each clip can be renamed inline -- `/rec/rename` sanitises the name, keeps the `.ts` extension, and refuses a clash or a file that is currently recording or playing. Clips can also be gathered into named **playlists** (stored in `~/atoll-recordings/playlists.json`, managed under their own collapsible menu -- create/rename/delete, add clips, reorder, remove): playing one streams every clip back-to-back from a single `playback-send.py` socket with no teardown between files, so the whole list goes out as one continuous stream (the receiver rides the PCR discontinuity at each boundary and resumes at the next keyframe), optionally looping the list.
...
**H.264 / H.265 (BCP-006-02).** The rig's H.264 and H.265 elementary streams (`h264-send.sh` / `h265-send.sh`, NVENC -> `rtph264pay` / `rtph265pay`) are made standards-clean the same way: `codec-nmos.py` (`atoll-codec-nmos`) registers each as an IS-04 coded-video Flow and serves an RFC 6184 / RFC 7798 SDP whose `fmtp` carries the **real** parameter sets, captured live off the wire (the H.265 profile_tier_level is read from the de-emulated SPS RBSP, so `level-id` is correct). Verified: `profile-level-id=42c01f` for H.264, `profile-id=2 level-id=93` for H.265, both with matching `sprop-*` sets, discoverable in the inspector.
...
> **Conformance scorecard.** Live AMWA `nmos-testing` results across the rig are aggregated in a dashboard artifact, reproduced by `pc/conformance-run.sh`. Current: **all 8 suites pass every applicable test** — IS-11 92/0, IS-12 40/40 (custom check, incl. a vendor worker that drives the rig), IS-05 62/0, IS-07 19/0, IS-04-03 17/0 (incl. the mDNS peer-to-peer test), IS-09 5/0, IS-08 31/0, IS-10 31/31 (custom check).
...
9. **IS-11 stream compatibility (full).** `is11-nmos.py` (`atoll-is11`, `:8107`) implements the AMWA IS-11 Stream Compatibility Management API from scratch, on a self-contained node that also self-hosts its own IS-04 Node API and a minimal IS-05 Connection API (the specs IS-11 builds on). It exposes one sender, receiver, input and output: the sender reports status and takes active/supported parameter constraints; applying a grain_rate constraint actually retunes the flow (the Node API flow's `grain_rate` follows) and clearing it restores the default. The input supports EDID -- a Base EDID PUT is validated (header + 128-byte-block checksum, 400 on garbage) and the Effective EDID is adjusted to the sender's active caps. Resource `version`s are stable and only bump on real change (EDID, constraints, activation), which controllers/tests poll on. **Verified with AMWA `nmos-testing` IS-11-01: 92 pass, 0 fail** -- every runnable test; the remaining 26 disabled / 14 could-not-test / 3 N/A need real hardware EDID, audio flows or multiple senders/receivers a software demo can't present.

10. **IS-12 control protocol + MS-05 model.** `is12-nmos.py` (`atoll-is12`, HTTP `:8108`, ncp WebSocket `:8109`) implements the AMWA IS-12 NMOS Control Protocol and the MS-05-02 object model from scratch. No WebSocket library is installable on this box, so the RFC 6455 codec is the hand-rolled one from `is07-tally.py`; over it the IS-12 messages flow -- Command (0) -> CommandResponse (1), Subscription (3) -> SubscriptionResponse (4), faults -> Error (5). The device model is the standard root **NcBlock** (oid 1) owning an **NcDeviceManager** (oid 2) and an **NcClassManager** (oid 3); every object answers generic Get/Set/GetSequenceItem/GetSequenceLength by property id, NcBlock answers GetMemberDescriptors, and the Class Manager answers GetControlClass / GetDatatype and exposes the controlClasses / datatypes properties -- served from the canonical MS-05-02 descriptors vendored in `pc/ms05-models/` (6 classes + 58 datatypes), with class inheritance merged on request. Beyond describing the device, IS-12 now **drives** it: a vendor **NcAtollRigControl** worker (oid 4, an NcWorker subclass, classId `[1,2,-1,1]` -- authority key -1 for "no OUI" per MS-05-02) is a third member of the root block, and setting its properties over the control WebSocket actuates the rig -- `program` (Set cuts the program bus to a source), `programLabel` (read-only), `avSyncMs` (Set moves the A/V-sync knob) -- with an invalid source rejected `ParameterError 407` and the read-only label `Readonly 405`. The guided demo cuts a source through IS-12 to show it. The AMWA `nmos-testing` **IS-12-01** suite requires the interactive Testing Facade ("No Override URL(s) specified" without it), so as with IS-10 the automated CLI pass isn't available here; `pc/is12-conformance.py` instead drives the ncp endpoint the way the suite's automatic tests would -- message framing, the full model walk, the class manager + inheritance, and every error status (BadOid 404, Readonly 405, PropertyNotImplemented 502, MethodNotImplemented 501) plus the NcAtollRigControl reads/writes that drive the rig: **40/40 pass**.

11. **Real controller interop (IS-04 discovery + IS-05 connect).** `pc/nmos-controller.py` is an *independent* controller with no rig-specific glue -- it speaks only the standard APIs. Point it at any IS-04 Query API and it lists the senders/receivers/devices it discovers, resolves a chosen receiver's IS-05 Connection API from that receiver's device `sr-ctrl` control, pulls the sender's SDP from its `manifest_href`, `PATCH`es the receiver's `staged` endpoint with `{sender_id, master_enable, transport_file, activation:{mode:"activate_immediate"}}`, and confirms the receiver's `/active` now reflects the sender. **Verified against the third-party nmos-cpp reference registry**: run from a clean container on the media network, the controller discovered the rig through that reference Query API and connected the Atoll **H.264 sender -> Atoll IS-11 receiver** -- `PATCH /staged` returned 200 and `/active` showed `master_enable=true` with the sender's id. That proves the plane is driven by the *standard* control surface, not a private path: any conformant controller (or the reference registry itself) can discover and route the rig. And it works **cross-vendor**, not just Atoll-to-Atoll: with the IS-11 demo sender's SDP retuned to the nmos-cpp reference receiver's caps (`easy-nmos-node/receiver/v0` requires 1920x1080i25 YCbCr-4:2:2 video/raw), the same controller connected the **Atoll IS-11 sender into the reference v0 receiver** -- `PATCH /staged` 200 and the reference node's `/active` shows the Atoll sender's id. An Atoll sender flowing into a third-party (nmos-cpp) receiver, negotiated through the reference registry by an independent controller.
