...
| Loudness meter | `pc/loudness.py` (+ `pc/bs1770.py`) | `atoll-loudness` | `:8104` | EBU R128 / ITU-R BS.1770-4 loudness on the **program** audio (follows the active source / Program Out). Momentary / Short-term / gated Integrated LUFS + an EBU R128 in-spec check (−23 LUFS ±1 LU). numpy-only K-weighting (FIR of the BS.1770 biquads). Serves JSON + a broadcast readout; the analyser header shows it too. |
