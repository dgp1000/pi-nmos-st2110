...
#   layout=multi  -> 2x2 mosaic (HEVC, Pi raw, JPEG-XS placeholder, clock)
#
# Run in a LOCAL WSL terminal:  bash pc/output-render.sh [SCREEN]   (SCREEN=2 default; 0=primary)
set -uo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$DIR/hevc-stream-env.sh"          # HEVC_IFACE = island NIC (auto-detected by IP)
...
# HEVC tile/full decode (NVDEC -> 30fps -> scale to WxH); caller terminates the TS audio pad.
hevc_video() { echo "udpsrc address=239.10.10.65 port=5010 multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 ! tsdemux name=hd hd. ! h265parse ! queue ! nvh265dec ! cudadownload ! videorate ! video/x-raw,framerate=30/1 ! videoconvert ! videoscale ! video/x-raw,width=$1,height=$2"; }
...
build_pipeline() {   # $1=layout  $2=active
  case "$1" in
    single)
      case "$2" in
        hevc) echo "gst-launch-1.0 -q udpsrc address=239.10.10.65 port=5010 multicast-iface=$IFACE auto-multicast=true buffer-size=8388608 ! tsdemux name=d d. ! h265parse ! queue ! nvh265dec ! cudadownload ! videoconvert ! videoscale ! video/x-raw,width=1920,height=1080 ! videoconvert ! waylandsink fullscreen=true sync=true d. ! mpegaudioparse ! queue ! mpg123audiodec ! audioconvert ! audioresample ! autoaudiosink sync=true" ;;
        ...
        jxs)  echo "ffmpeg -hide_banner -loglevel error -fflags nobuffer -flags low_delay -i 'udp://239.10.10.22:5008?localaddr=10.10.10.2&overrun_nonfatal=1&fifo_size=5000000&buffer_size=67108864' -f rawvideo -pix_fmt yuv422p - | gst-launch-1.0 -q fdsrc ! rawvideoparse format=Y42B width=1920 height=1080 framerate=60000/1001 ! videoconvert ! waylandsink fullscreen=true sync=false" ;;
      esac ;;
    side)
      echo "gst-launch-1.0 -e compositor name=mix background=black sink_0::xpos=0 sink_0::ypos=270 sink_1::xpos=960 sink_1::ypos=270 ! video/x-raw,width=1920,height=1080 ! videoconvert ! waylandsink fullscreen=true sync=false \
        $(hevc_video 960 540) ! textoverlay text='PC HEVC 4K' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! mix.sink_0 \
    ...
    multi)
      echo "gst-launch-1.0 -e compositor name=mix background=black sink_0::xpos=0 sink_0::ypos=0 sink_1::xpos=960 sink_1::ypos=0 sink_2::xpos=0 sink_2::ypos=540 sink_3::xpos=960 sink_3::ypos=540 ! video/x-raw,width=1920,height=1080 ! videoconvert ! waylandsink fullscreen=true sync=false \
        ...
        hd. ! mpegaudioparse ! fakesink sync=false \
        $(raw_video 960 540) ! textoverlay text='Pi raw 2110-20' valignment=top halignment=left xpad=14 ypad=10 font-desc='$F' shaded-background=true ! mix.sink_1 \
        videotestsrc pattern=blue is-live=true ! video/x-raw,width=960,height=540,framerate=30/1 ! textoverlay text='PC JPEG-XS  (no sender)' valignment=center halignment=center font-desc='$F' shaded-background=true ! mix.sink_2 \
