...
run(){
  ...
  sudo -n docker exec -w /home/nmos-testing nmos-score bash -lc "timeout 260 python3 nmos-test.py suite $1 --host $2 --port $3 --version $4 ${5:-} --selection all --output /tmp/$1.json >/tmp/$1.out 2>&1"
  sudo -n docker exec nmos-score python3 /tmp/scoreparse.py "$1" >> /tmp/score.tsv 2>/dev/null
}
run IS-05-01 192.168.4.85 8107 v1.1
run IS-08-01 192.168.4.85 8094 v1.0 '--selector ""'
